...
const TestComponent = ({ children }: { children: (authData: ReturnType<typeof useAuth>) => ReactElement }) => {
    ...
    <MockedProvider mocks={[...getLoginRequestMocks(email, password)]}>
      <AuthProvider>
        <TestComponent>
          {(authData) => (
            <div>
              {authData.isSignedIn === undefined ? 'Loading...' : authData.isSignedIn ? 'Logged In' : 'Logged Out'}
...
test('provides error after login error', async () => {
  ...
  const container = render(
    <MockedProvider
      mocks={[sessionInfoQuerySignedOutMock.build(), loginQueryMock, sessionInfoQuerySignedOutMock.build()]}
    >
      <AuthProvider>
        <TestComponent>
          {(authData) => (
  ...
  await waitFor(() => expect(container.getByText(errorMessage)).toBeInTheDocument());
});
