    ...
    <div className="relative text-left flex flex-col-reverse">
        ...
        >
          <span className="flex w-full justify-between items-center">
            <span className="flex items-center">
              <span>
                <img
                  className={`inline-block rounded-full ${props.desktop ? 'h-9 w-9' : 'h-10 w-10 '}`}
                  src={profile.image}
                  alt=""
                />
              </span>
              <span className="ml-3">
                <p className={`${props.desktop ? 'text-sm' : 'text-base'} font-medium text-gray-700`}>
                  {profile.firstName} {profile.lastName}
                </p>
                <p className={`${props.desktop ? 'text-xs' : 'text-sm'} font-medium text-gray-500`}>{profile.title}</p>
              </span>
            </span>
      ...
      {isDropdownOpen && (
        <ClickAwayListener onClickAway={() => setIsDropdownOpen(false)}>
          <Dropdown profile={profile} onClick={() => setIsDropdownOpen(false)} />
        </ClickAwayListener>
      )}
    </div>
  );
