...
const UserSelect = ({ profileList, value, onChange }: Props) => {
  ...
  return (
    <Listbox value={selectedPerson} onChange={(p) => onChange(p.id)}>
      {({ open }) => {
        const selectedFullName = `${selectedPerson.firstName} ${selectedPerson.lastName ?? ''}`;
        return (
          <div className="mt-1 relative">
            <Listbox.Button className="relative w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
              <span className="flex items-center">
                <img src={selectedPerson.image} alt={selectedFullName} className="flex-shrink-0 h-6 w-6 rounded-full" />
            ...
            >
              <Listbox.Options className="max-h-56 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                {sortedProfiles.map((profile) => (
                  <Listbox.Option as={Fragment} key={profile.id} value={profile}>
                    {({ active, selected }) => (
                      <li
                        className={`${
                      ...
                      >
                        <div className="flex items-center">
                          <img src={profile.image} alt="" className="flex-shrink-0 h-6 w-6 rounded-full" />
                          <span className={`ml-3 block ${selected ? 'font-semibold' : 'font-normal'} truncate`}>
